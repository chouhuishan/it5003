...
class SLL:
    ...
    def appendleft(self, v):
        ...
        if self._head == None:
            self._tail = vtx  # if previously it was an empty SLL, then tail = head too
    ...
    def append(self, v):
        if self._head == None:  # an important corner case
            ...
            self._tail.next = (
                vtx  # just link this, as tail is the i = (N-1)-th item, O(1)
            )
            self._tail = vtx  # now update the tail pointer, O(1)

    def front(self):
        if self._head == None:
            return None  # avoid crashing when the SLL is empty
    ...
    def popleft(self):
        ...
        if self._head == None:
            self._tail = None  # if the SLL is now becomes empty, then tail = NULL too
...
class MyStack:  # stacked will be last in first out; first one to come out is the last one placed inside
    def __init__(self):
        self.sll = SLL()

    def peek(self):
        return self.sll.front()

    def push(self, v):
        self.sll.appendleft(v)
...
class MyQueue:  # queue will be first in first out; first one to come out is the first one place inside
    def __init__(self):
        self.sll = SLL()
